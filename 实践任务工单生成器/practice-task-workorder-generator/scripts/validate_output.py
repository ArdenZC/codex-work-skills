"""Output structure and canonical Content-to-DOCX fidelity checks."""

from __future__ import annotations

import argparse
import json
import re
from pathlib import Path
from typing import Any

from docx import Document

from content_contract import canonicalise_content, normalise_text


class WorkOrderOutputError(ValueError):
    pass


_UNANSWERED_RESULT_RE = re.compile(r"^[\s_＿\-—–·.…。．]+$")


def _cell_text(cell: Any) -> str:
    return "\n".join(paragraph.text for paragraph in cell.paragraphs).strip()


def _table_text(table: Any) -> str:
    return "\n".join(_cell_text(cell) for row in table.rows for cell in row.cells)


def _is_unanswered_result_area(value: str) -> bool:
    """Allow an empty or placeholder-only student writing area."""

    normalized = value.strip()
    return not normalized or bool(_UNANSWERED_RESULT_RE.fullmatch(normalized))


def _item_text(value: Any) -> str:
    if isinstance(value, dict):
        return normalise_text(value.get("text", ""))
    return normalise_text(value)


def _visible_document_text(document: Any) -> str:
    paragraphs = "\n".join(paragraph.text for paragraph in document.paragraphs)
    tables = "\n".join(_table_text(table) for table in document.tables)
    return f"{paragraphs}\n{tables}"


def validate_document(path: Path, content: dict[str, Any] | None = None) -> dict[str, Any]:
    errors: list[str] = []
    try:
        document = Document(str(path))
    except Exception as exc:
        return {"status": "fail", "errors": [f"DOCX cannot be opened: {exc}"], "metrics": {}}
    canonical = None
    if content is not None:
        canonical = canonicalise_content(content, compatibility=content.get("content_contract_version") == "1.0")
    tables = document.tables
    if len(tables) != 3:
        errors.append(f"expected exactly 3 top-level tables, got {len(tables)}")
        return {"status": "fail", "errors": errors, "metrics": {"tables": len(tables)}}
    headers = [normalise_text(_cell_text(cell)) for cell in tables[0].rows[0].cells]
    expected_headers = ["任务对象", "任务项", "任务描述", "任务结果", "分值"]
    if headers != expected_headers:
        errors.append(f"work-order headers differ: expected {expected_headers}, got {headers}")
    if len(tables[0].rows) < 2:
        errors.append("work-order table has no attendance row")
    else:
        attendance = tables[0].rows[1]
        if normalise_text(_cell_text(attendance.cells[1])) != "课堂考勤":
            errors.append("attendance row title must remain 课堂考勤")
        if normalise_text(_cell_text(attendance.cells[4])) != "10":
            errors.append("attendance score must be 10")
    expected_items = canonical.get("task_items", []) if canonical else []
    if canonical and len(tables[0].rows) != 2 + len(expected_items):
        errors.append(
            f"task row count differs: expected {len(expected_items)}, got {max(0, len(tables[0].rows) - 2)}"
        )
    task_rows = tables[0].rows[2:]
    for index, row in enumerate(task_rows):
        if index >= len(expected_items):
            break
        item = expected_items[index]
        if normalise_text(_cell_text(row.cells[1])) != normalise_text(item["title"]):
            errors.append(f"task row {index + 1} title does not match WorkOrder Content 1.1")
        description_text = normalise_text(_cell_text(row.cells[2]))
        if normalise_text(item["description"]) not in description_text:
            errors.append(f"task row {index + 1} description does not match WorkOrder Content 1.1")
        for key in ("tools_or_materials", "steps", "deliverables", "acceptance_criteria"):
            for value in item[key]:
                if _item_text(value) not in description_text:
                    errors.append(f"task row {index + 1} missing {key} value")
        if normalise_text(_cell_text(row.cells[4])) != str(item["score"]):
            errors.append(f"task row {index + 1} score does not match WorkOrder Content 1.1")
        if not _is_unanswered_result_area(_cell_text(row.cells[3])):
            errors.append(f"task row {index + 1} student result cell contains answer text")

    work_order_text = _table_text(tables[0])
    if canonical:
        profile = canonical["course_profile"]
        metadata_fields = (
            ("course_name", profile["course_name"]),
            ("major", f"专业：{profile['major']}"),
            ("audience", f"对象：{profile['audience']}"),
            ("practice_hours", f"实践学时：{canonical['practice_hours']}"),
        )
        for field, expected in metadata_fields:
            if normalise_text(str(expected)) not in normalise_text(work_order_text):
                errors.append(f"work-order metadata missing {field}")
        for value in canonical.get("safety_or_compliance", []):
            if normalise_text(value) not in normalise_text(work_order_text):
                errors.append("work-order metadata missing safety_or_compliance value")
        title_text = "\n".join(paragraph.text for paragraph in document.paragraphs)
        if normalise_text(canonical["task_title"]) not in normalise_text(title_text):
            errors.append("document title is missing task_title")

        # Internal provenance IDs belong in JSON/QA metadata and filenames, not
        # in the student-facing form or Office core properties.
        visible_text = _visible_document_text(document)
        internal_ids = [canonical.get("task_id"), canonical.get("project_id"), *canonical.get("lesson_ids", [])]
        for identifier in internal_ids:
            identifier = normalise_text(identifier)
            if identifier and identifier in visible_text:
                errors.append(f"student-facing DOCX exposes internal identifier: {identifier}")
        properties = document.core_properties
        core_text = " ".join(str(value or "") for value in (properties.title, properties.subject, properties.keywords))
        for identifier in internal_ids:
            identifier = normalise_text(identifier)
            if identifier and identifier in core_text:
                errors.append(f"DOCX core properties expose internal identifier: {identifier}")
        if normalise_text(canonical["task_title"]) not in normalise_text(properties.title or ""):
            errors.append("DOCX core title is missing task_title")

    student_text = _table_text(tables[1])
    full_text = _visible_document_text(document)
    for marker in ("自我评价", "小组评价", "教师评价"):
        if marker not in student_text:
            errors.append(f"student evaluation fixed marker missing: {marker}")
    for marker in ("20%", "30%", "50%"):
        if marker not in full_text:
            errors.append(f"student evaluation fixed weight missing: {marker}")
    teacher_text = _table_text(tables[2])
    if len(tables[2].rows) != 4:
        errors.append(f"teacher evaluation table must have 4 rows, got {len(tables[2].rows)}")
    if teacher_text.count("A") < 3 or teacher_text.count("B") < 3 or teacher_text.count("C") < 3:
        errors.append("teacher evaluation fixed A/B/C criteria are incomplete")
    if canonical:
        score_total = 10 + sum(int(item["score"]) for item in expected_items)
        if score_total != 100:
            errors.append(f"work-order total score must be 100, got {score_total}")
    return {
        "status": "pass" if not errors else "fail",
        "errors": errors,
        "metrics": {
            "tables": len(tables),
            "task_rows": len(task_rows),
            "student_result_cells_blank": sum(not _cell_text(row.cells[3]) for row in task_rows),
            "student_result_cells_unanswered": sum(_is_unanswered_result_area(_cell_text(row.cells[3])) for row in task_rows),
        },
    }


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--input", required=True, type=Path)
    parser.add_argument("--content-json", type=Path)
    parser.add_argument("--json", action="store_true")
    args = parser.parse_args()
    content = None
    if args.content_json:
        value = json.loads(args.content_json.read_text(encoding="utf-8"))
        content = value[0] if isinstance(value, list) else value
    report = validate_document(args.input, content)
    if args.json:
        print(json.dumps(report, ensure_ascii=False, indent=2))
    else:
        print(report["status"])
        for error in report["errors"]:
            print(f"- {error}")
    return 0 if report["status"] == "pass" else 1


if __name__ == "__main__":
    raise SystemExit(main())
