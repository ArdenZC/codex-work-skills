...
实践课 HTML 生成器可通过同名 namespaced marker 安装 Codex/Claude/Gemini/Copilot/Aider/Cursor/Cline/Continue/Windsurf/OpenCode 规则；规则要求优先消费 Courseware Content Contract 1.0、保留 `source_slide_ids`，并在生成后运行合同、内链和浏览器 QA。它与旧实践任务工单 Skill 保持边界，不复制旧 Hardening 体系。
