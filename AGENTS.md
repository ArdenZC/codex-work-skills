...
模板发布工具的 canonical source、archive、installed version 和 GitHub Release 是不同边界。`release` 只从 canonical package 生成既有三文件 archive 和不含绝对路径的 plan；`verify-release` 必须通过 metadata、实际 ZIP、sidecar、依赖闭包和当前 Git-tracked owner full validation。`install`/`upgrade` 不得写 canonical、执行 bundle 代码或覆盖 immutable version，必须使用 O_EXCL lock、stage/atomic exchange 和失败恢复；`rollback` 只允许切换到更低的已安装版本。默认安装根只允许 `installed/template-packages/`，外部路径必须解析后完全独立，所有 symlink、Windows 8.3 和 overlap 检查 fail closed。真实 tag/Release 只能由 clean master 上的 `workflow_dispatch` 创建，普通 CI 和本地测试不得创建远程资源。
