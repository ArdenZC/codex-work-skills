...
模板发布工具的 canonical source、archive、installed version 和 GitHub Release 是不同边界。`release` 可从 canonical 或 release workspace package 生成既有三文件 archive 和不含绝对路径的 plan；Release owner 按 `template_id`、format 和 `owner_skill` 解析，不要求同版本 canonical，`template_sha256` 绑定 Release 自身 entry package。`verify-release` 必须通过 metadata、实际 ZIP、sidecar、依赖闭包和当前 Git-tracked owner full validation，并在解压前执行 ZIP entry/size/total/compression limits 和流式 SHA。`install`/`upgrade` 不得写 canonical、执行 bundle 代码或覆盖 immutable version，必须使用 O_EXCL lock、stage/atomic exchange 和失败恢复；`rollback` 是已安装版本之间的 active/previous 状态切换，连续调用可以 toggle。active state 兼容性由 schema_version 决定，tool version 只需是严格 ASCII semver。默认安装根只允许 `installed/template-packages/`，外部路径必须解析后完全独立，所有 symlink、Windows 8.3 和 overlap 检查 fail closed。真实 tag/Release 只能由 clean master 上的 `workflow_dispatch` 创建；发布前必须 live 查询远程 master，远程 asset 必须与 local plan 逐字节一致，远程不确定状态只允许 fail closed，普通 CI 和本地测试不得创建远程资源。
