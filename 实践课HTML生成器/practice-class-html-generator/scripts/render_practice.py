...
def render_teacher_guide(content: dict[str, Any]) -> str:
    ...
    body = f'''<section class="hero"><div class="eyebrow">课堂控制台</div><h2>教师指南</h2><p>{text_block(teacher["purpose"])}</p><div class="callout"><strong>理论—实践桥：</strong>{text_block(teacher["theory_bridge"])}</div></section><section class="section"><h2>建议节奏</h2><div class="card"><ol>{timing}</ol></div></section><section class="section"><h2>任务观察点</h2><div class="card-grid">{guidance}</div></section><section class="section"><h2>常见错误与干预</h2><div class="card warm"><ul>{errors}</ul></div></section><section class="section"><h2>按课堂状态调整</h2><div class="card-grid">{"".join(f"<div class=\"card soft\"><p>{text_block(item)}</p></div>" for item in teacher["pace_adjustments"])}</div></section><section class="section"><h2>结束检查</h2><div class="card blue"><ul>{"".join(f"<li>{text_block(item)}</li>" for item in teacher["closing_checks"])}</ul></div></section>'''
