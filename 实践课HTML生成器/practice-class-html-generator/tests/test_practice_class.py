...
class PracticeClassPackageTests(unittest.TestCase):
    ...
    def test_generation_emits_six_module_pages_and_output_qa(self) -> None:
        with tempfile.TemporaryDirectory(prefix="practice-class-package-") as temp:
            ...
            self.assertEqual(report["status"], "pass", report)
            self.assertEqual({path.name for path in output.iterdir()}, {"student", "teacher", "student-package", "teacher-package", "practice-content.json", "qa-report.json"})
            self.assertTrue((output / "student-package" / "student-manifest.json").is_file())
            self.assertTrue((output / "teacher-package" / "full-practice-content.json").is_file())
            ...
            self.assertEqual(len(list(output.rglob("*.html"))), 12)
