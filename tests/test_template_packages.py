...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_template_package_ci_runs_on_main_push(self) -> None:
        ...
        self.assertIn("install libreoffice", workorder_text)
        self.assertIn("render.status", workorder_text)
        self.assertIn('item.get("status") != "pass"', workorder_text)
        self.assertIn("soffice.com", workorder_text)
        ...
        self.assertIn("GENERATION_TEMPLATE_TESTS", shard_runner)
        manifest_result = run_script(ROOT / ".github" / "scripts" / "run_gradebook_tests.py", "--list")
        self.assertEqual(manifest_result.returncode, 0, manifest_result.stdout + manifest_result.stderr)
        self.assertIn("contracts: 34 tests", manifest_result.stdout)
        self.assertIn("generation: 26 tests", manifest_result.stdout)
        self.assertIn("total: 60 tests", manifest_result.stdout)
