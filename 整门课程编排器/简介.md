...
Phase 1.1 明确区分 `required`、`planned`、`observed`：planner 声明不是最终证据。最终 Courseware DOM、Practice starter/manifest/QA 和 contact-sheet 文件必须实际观察；浏览器不可用时状态保持 `WHOLE_COURSE_ARCHITECTURE_BLOCKED`，不会把 fallback thumbnail 写成 PASS。
