...
- 2.2.2 hotfix 要求把确认后的课程名称、专业、授课对象、总课时、理论/实践课时和组织方式冻结到 `confirmed_course_info`，若快照存在则 JSON 与 DOCX 只能使用这些确认值，正文 Agent 不得重写。`course_materials.textbook`、课次 `resources` 与 `reference_pool` 彻底分离；教材、PPT、课件、案例表/案例数据、内部教学资源、任务单和设备名不得进入 references，`allow_textbook_as_reference` 不再提供例外。Lesson DOCX 只承载理论课时，实践课时不进入 Lesson DOCX；`sum(lesson.hours)=theory_hours`，实践学时仍参与课程总账，理论与实践合计等于总课时。理论课次按 `ceil(theory_hours/default_hours)` 计算，余数课时必须保留，不能四舍五入。
- 2.2 每个理论 Lesson 必须引用至少一项 `reference_ids`；有联网能力时确认后必须先检索真实公开来源，再建立 reference pool；references 是可阅读、查阅、引用或作为课程依据的文献/文档，resources 是工具、设备、环境和材料。引用目录按用户资料、国内出版物/高校资料、国家/行业/职业标准、国内权威文档、国外经典优先建立；不足时少写，不得用教材或内部资源填数，也不得虚构作者、出版社、ISBN、版次或标准编号。合法 references 可跨课复用，同课重复 ID、未解析 ID、占位文献和纯资源名必须失败。
- 正文 formatter 会删除末尾机械的“聚焦：xxx”“聚焦于：xxx”“围绕：xxx”“针对：xxx”及同形括号尾缀，正文应自然表达主题。
- 2.2.2 参考文献必须保留责任信息：`book` 要有真实作者/编者、出版社和年份；`formal_course_document` 要有真实责任教师/课程团队、开课高校 `institution` 和课程平台/出版社 `publisher`，不能只写平台名称。优先混合正式出版物与高校公开课程，教材仍严格排除。
