...
- 四个核心字段缺失时必须在同一摘要显示“待补充”，总课时只能来自用户明确事实；未明确提供的理论/实践结构、组织方式和工单偏好必须显示“待确认”，不得默认 50/50、综合式或“否”。专业/授课对象若为推断，必须标注“当前理解 / 如不准确请修改”。Intake 未确认前不得提前确定项目、课次结构、实践任务数量、正文或 DOCX；用户确认后进入 `INTAKE_CONFIRMED`，直接完成资料检索、outline、Content Contract 2.1、Practice Task Contract、QA 和 DOCX。
