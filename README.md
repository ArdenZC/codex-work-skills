...
`discover` 只读取 manifest；`validate` 默认调用所属 Skill 的真实 `scripts/validate_template.py`，`--identity-only` 只代表身份检查而不是完整通过。`scaffold` 默认只生成同 major/minor 的新 patch，`--allow-unsupported-minor` 只能在 canonical 树外生成不可晋级开发包。`promote` 会先完整校验，再把新包原子放入 canonical 树并运行仓库级模板校验；`archive` 生成稳定排序、固定时间和权限的 ZIP、SHA-256 sidecar 与元数据。所有变更命令都拒绝覆盖已有目标，支持 `--dry-run` 的命令不会创建文件。详细字段和生命周期见 [`docs/template-package-authoring.md`](docs/template-package-authoring.md)。
