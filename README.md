...
发布和安装生命周期见 [`docs/template-package-release.md`](docs/template-package-release.md)。canonical 模板、三文件 archive、installed 版本和 GitHub Release 是四类不同对象；installed 版本 immutable，rollback 只允许切换到更低的已安装版本，且安装器不会执行 bundle 中的代码。发布 workflow 只从 clean `master` 手动触发，不覆盖已有 tag、Release 或 asset。
